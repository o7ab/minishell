...
/*   Updated: 2023/01/30 03:58:37 by omar             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_separat_key(int m)
{
	...
	{
		if (!(g_info->cmd->s_cmd[m][i] == '_') &&
			!(ft_isalnum(g_info->cmd->s_cmd[m][i])))
		{
			printf("%d wrong\n", i);
			exit(0);
		}
...
void	ft_export(void)
{
	...
	{
		if (g_info->cmd->s_cmd[j][0] == '_' ||
			ft_isalpha(g_info->cmd->s_cmd[j][0]) == 1)
			ft_separat_key(j);
		else
		{
